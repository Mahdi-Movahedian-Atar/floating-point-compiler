    ...
    pub fn tokenizer(line: String, line_num: u32) -> (Vec<FPoint>, Vec<(i16, Token)>, i8) {
        ...
        while i < length {
            ...
            match chars[i] {
                '\'' => {
                    ...
                    if i + 1 < length {
                        ...
                        if token.as_bytes()[2] as char == '\'' {
                            ...
                            tokens.1.push((
                                token_hash,
                                Token::Lit(VariableType::Character(token.as_bytes()[1] as char)),
                            ));
                            token = String::new();
                            continue;
                        }
                    ...
                    if is_float {
                        match token.parse::<f32>() {
                            Ok(val) => {
                                ...
                                tokens
                                    .1
                                    .push((token_hash, Token::Lit(VariableType::Float(val))));
                            }
                            Err(_) => {
                                token_insert!(
                                    Error,
                                    "Could no parse the float type",
                                    tokens,
                                    token,
                                    line_num
                                );
                        ...
                        match token.parse::<i32>() {
                            Ok(val) => {
                                ...
                                tokens
                                    .1
                                    .push((token_hash, Token::Lit(VariableType::Integer(val))));
                            }
                            Err(_) => {
                                token_insert!(
                                    Error,
                                    "Could no parse the integer type",
                                    tokens,
                                    token,
                                    line_num
                                );
                ...
                '+' => token_insert!("+", tokens, token, line_num),
                '-' => {
                    if i + 1 < length && chars[i + 1].is_digit(10) {
                        i += 1;
                        let mut is_float = false;
                        while i < length {
                            match chars[i] {
                        ...
                        token = String::new();
                    }else { token_insert!("-", tokens, token, line_num) }
                },
                '*' => {
                    if i + 1 < length {
                        if chars[i + 1] == '/' {
